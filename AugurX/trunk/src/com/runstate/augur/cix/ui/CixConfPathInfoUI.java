...
import com.runstate.augur.cix.pathinfo.*;
...
public class CixConfPathInfoUI extends PathInfoUI {
	JTable topictable;
...
//		parseText();
		misctext.setHTML(Controller.getController().wrapWithStyle("", ((CixConfPathInfo)getPathInfo()).getConfnote()));
	...
	class PartTableModel implements TableModel {
		...
		public int getRowCount() {
			...
			if (((CixConfPathInfo)getPathInfo()).getParticipants() == null) return 0;
			return ((CixConfPathInfo)getPathInfo()).getParticipants().size();
		}
		...
		public Object getValueAt(int rowIndex, int colIndex) {
			CixConfPathInfo.Part p=((CixConfPathInfo)getPathInfo()).getParticipants().get(rowIndex);
			switch (colIndex) {
				case 0: return p.mod;
	...
	class TopicTableModel implements TableModel {
		...
		public int getRowCount() {
			if (getPathInfo() == null) return 0;
			if (((CixConfPathInfo)getPathInfo()).getTopics() == null) return 0;
			return ((CixConfPathInfo)getPathInfo()).getTopics().size();
		}
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			CixConfPathInfo.Topic te=((CixConfPathInfo)getPathInfo()).getTopics().get(rowIndex);
			switch (columnIndex) {
				case 0:
