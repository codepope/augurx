...
public class TwixRootPathInfo extends TwixPathInfo implements Serializable {
	...
	public TwixRootPathInfo(Long doorid,Long bundleid) {
	...
	public String getUIClassName() {
		return "com.runstate.augur.twix.ui.TwixRootPathInfoUI";
	}
	...
	private void parseRootPathInfo(String conflist) {
		...
		for(String line:lines) {
			if(sublist) {
				if((line.equals(" #####AUGURBREAK2#####")) || (line.equals(" #####AUGURBREAK#####"))){
					sublist=false;
				}
				else {
				...
				if(m.matches()) {
					...
					boolean exists=Controller.getController().getGallery().getBundleManager().getBundleExists("/twix/"+name);
...
//Doesn't seem to be ConfNote in Twix                                
