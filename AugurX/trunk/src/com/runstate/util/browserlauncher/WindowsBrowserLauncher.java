...
public class WindowsBrowserLauncher {
  ...
  private final static boolean _debug = false;
    ...
    int srch_index = inSource.indexOf(search);

    while(srch_index != -1) {
      inSource = inSource.substring(0,srch_index) +
        replacement +
        inSource.substring(srch_index + search.length());

      srch_index = inSource.indexOf(search, srch_index + replacement.length());
    }
   ...
   *
   * @throws - An Exception with info on what failed.
   */
  ...
  public static void main(String[] args) {
    ...
    if(args.length == 2) {
      try {
        ...
        System.err.println("Failed to launch URL " + args[1]);
      }
